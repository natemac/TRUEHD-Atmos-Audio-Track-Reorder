...
set "SCRIPT_DIR=%~dp0"
set "PS_SCRIPT=%SCRIPT_DIR%mkv-reorder.ps1"
...
if "%~1"=="" (
    echo.
    echo Usage:
    echo   Drag an MKV file onto this batch file, or run:
    echo   mkv-reorder.bat "path\to\file.mkv" [--no-backup] [--dry-run]
    echo.
    pause
    exit /b 1
)

powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%PS_SCRIPT%" -FilePath "%~1" %2 %3 %4 %5 %6
